...
class SystemSessionController {
    ...
    def show(Long id) {
        ...
        def systemSession =systemSessionService.get(id)

        if (request.xhr) {
            render(template: view, model: [systemSession: systemSession])
        } else {
            respond systemSession
        }
    }
    ...
    def save(SystemSession systemSession) {
        ...
        try {
            ...
            flash.message = systemSession.errors
        }
    ...
    def importFromJsonFile(String fileName) {
        // 先清空
        SystemSession.list().each { e ->
            systemSessionService.delete(e.id)
        }

        def jsonFile = new File(fileName)
        if (jsonFile.exists()) {
            def json = jsonFile.text
            def querys = commonService.importFromJson(json, SystemSession.class)
            querys.each { e ->
                systemSessionService.save(e)
            }
        }
    ...
    def exportToJsonFile(fileName) {
        def fjson = commonService.exportObjects2JsonString(SystemSession.list())
